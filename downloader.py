...
def check_python_version(v):
    ...
    if sys.version_info[0] != int(v[0]):
        ...
        print('You are using {0}.{1}.{2} {3}'.format(sys.version_info[0], sys.version_info[1], sys.version_info[2],
                                                     sys.version_info[3]))
        sys.exit(1)
...
def request_content(url, time_out=60, debug=False):
    try:
        resp = requests.get(url, stream=True,  # enabling stream to avoid memory overrun for large files
                            timeout=time_out,
                            # timeout to throw error and avoid incomplete file downloads when lost connection
...
def download_file(path, f_url, parallel=0, t_out=30, stack_trace=False, aws_a_key=None, aws_s_key=None):
    ...
    if proto in ['http', 'https']:
        ...
        print('\nFinished downloading {0} from {1}\nFile Path:     {2}\nDownload Time: {3}'.format(file_name, url,
                                                                                                   file_path, t_finish))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--download', nargs='+',
                        help='Provide single or multiple space/comma separated files to download (ex. ./downloader.py -d http://domain.com/file1 https://domain.com/file2 s3://key/file1)')
    parser.add_argument('-p', '--path', help='Provide path where to save downloaded file/s. Default is script\'s path',
                        default='.')
    parser.add_argument('-c', '--concurrent', type=int,
                        help='Set number of downloads to run in parallel. Default is one at a time', default=0)
    parser.add_argument('-t', '--timeout', type=int,
                        help='Set inactive connection timeout for downloads (seconds). Default is 30 seconds',
                        default=30)
    parser.add_argument('-b', '--debug', action='store_true',
                        help='Enable debug mode to stacktrace errors. Default is False', default=False)
    parser.add_argument('-s', '--s3-credentials', action='store_true',
                        help='Enable this to be prompted for S3 credentials. Default is Disabled', default=False)
    parser.add_argument('-r', '--reveal-credentials', action='store_true',
                        help='Enable this to un-hide AWS credentials during input. Works together with "-s or --s3-credentials". Default is Disabled',
                        default=False)
